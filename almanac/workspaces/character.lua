...
Character.promo_use_negative = {"bld", "mov", "con", "wlv"}
Character.promo_job_cap = false
Character.promo_inline = true
Character.promo_minHP = false
    ...
    for i, k in pairs(util.text.stat_order(result)) do
        ...
        if #compare == 2 then
            ...
            if v.diff ~= 0 then
                local diff
                if (v.diff % 1 == 0) then
                    diff = string.format("+%s", v.diff)
                else
                    diff = string.format("+%.2f", v.diff)
                end
...
function Character:show_promo()
    ...
    if not self.minimal and promo ~= nil and self.job then
        ...
        for i, pair in ipairs(promo) do
            ...
            local text = ""
            -- Stats
            if self.promo_minHP then
                text = "Min. " .. promoted.data.base['hp'] .. ' HP | '                
            end
            text = text .. util.table_stats(bonus, {value_start = "+"})
