...
function module.growth_stats(v1, v2, lvl, args)
    local function step(v1, v2)
        ...
        return module.round_closest(v1 + v2)
    end
...
function Inventory:equip_item(item)
    ...
    for i, tbl in ipairs(self.item_func) do
        ...
        -- True Damage
        if tbl.name == "atk" and item:is_magic_true_damage() then
            -- Add magic true damage field
            stats["True Dmg"] = math.floor( data.unit.stats.mag / 3)
        end
        
        if tbl.name == "atk" and item:is_speed_true_damage() then
            -- Add speed true damage field
            stats["True Dmg"] = math.floor( data.unit.stats.spd / 3)
        end
