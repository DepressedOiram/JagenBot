...
function Character:final_base()
    ...
    if self.job.id ~= self.data.job then
        ...
        base.hp = base.hp + job.data.base.hp - self.job.data.base.hp 
...
function Character:show_sup()
    ...
    local text = "Units must be standing within 3 tiles to receive bonus Hit/Avoid/Crit/Crit Avoid.\n\n"
...
function Skill:get_icon()
    if self.data.name ~= nil then
        return string.format("database/fe4/images/skill/%s.png", self.data.name)
    end
end
