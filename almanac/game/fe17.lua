...
local function inventory_atk(data, unit, item)
    ...
    elseif item:is_art() and item.id ~= "dragonsfist" and item.id ~= "scrollcake" then
...
function Skill:get_emoji()
    -- combat arts emoji
    return pack:get("fe17_" .. tostring(self.data.icon))
...
function Character:get_rise_above()
    ...
    for key, value in pairs(self:final_growth()) do
        value = value / 100
        value = util.floor(value * lvl + 0.5)
...
function Ring:show_emblem()
    ...
    for key, value in pairs(self.data.rank) do
        ...
        for i, weapon in ipairs(value.aptitude) do
            aptitude = aptitude .. pack:get(weapon, "") .. string.format("%s (Lv. %s)\n", util.title(weapon),
                                                 key)
        end
        ...
        for i, item in ipairs(value.item) do
            ...
            items = items .. pack:get(item.data.type, "") .. string.format("%s (%s)\n",
                             item:get_name(), add)
        end
...
function Ring:show_bond()
    ...
    for key, value in pairs(self.data.rank) do
        ...
        infobox:insert( pack:get(key .. "_Bond").. key .. " Rank", util.table_stats(self:get_bonus(key), {value_start = "+"}), true)
    end
...
function Item:show()
    ...
    local function upgrade_price(data)
        ...
        text = text .. pack:get("G") .. data["Price"]
