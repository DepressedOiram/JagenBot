...
function Character:setup()
    if self.options.max or self.options.secret then
        ...
        if self:has_aide() or self:is_aided() then
            self.options.aide = true
        end
...
function Character:show_info()
    ...
    local infobox_footer = string.format("")
    -- chibis
    if #self.data.chibi > 0 then

        for i, p in ipairs(self.data.chibi) do
            infobox_footer = infobox_footer .. string.format("%s", heroes_pack:get(p))
        end
        infobox_footer = infobox_footer .. string.format("%s | Version %s", self.data.origin, self.data.version)
        infobox:insert("", infobox_footer)
    end
    --------------------
    -- Skill Page
    ...
    if self:is_emblem() then
        ...
        skillbox_desc = string.format("%s%s\n", heroes_pack:get("feh_emblem"), self:engage_effect()) .. skillbox_desc
    end
	...
	skill_field("Passive X", "X", self.data.passive.X)
    ...
    if #self.data.reference > 0 then
        ...
        for i, p in ipairs(self.data.reference) do
            ...
            if i > 13 then
                alts_text2 = alts_text2 .. string.format("%s | *%s*\n", ref:fancy_short(), ref.id)
            else
        ...
        miscbox:insert("Alts | Bot ID", alts_text2, false)
...
function Character:get_info()
    ...
    if self:is_aided() then
        text = text .. string.format("%sAided Hero\n", heroes_pack:get("aide", "💠"))
    end
...
---------------------
function Character:is_duo_harm()
...
function Character:engage_effect()
    return self.data['emblem']
end
...
function Character:is_duo()
    return (self.data.duo)
end
function Character:is_harmonized()
    return (self.data.harmonized)
end
...
function Character:duo_icon()
    return heroes_pack:get("feh_duo")
end
...
function Character:is_aided()
    return (self.data.type == "aided")
end

function Character:is_attuned()
    return (self.data.type == "attuned")
end
...
function Character:has_aide()
    return (self.data.aide)
end
...
function Character:get_portrait()
    if self.resplendent and self:has_resplendent() then
        return string.format("database/feh/images/heroes/%sR.png", self.id)
    
    else
        return string.format("database/feh/images/heroes/%s.png", self.id)
    end
end
...
function Skill:default_options()
    return {
        engage = false
    }
end

function Skill:setup()
    self.engage = self.options.engage
    if self.engage then
        self.engage = Engage:new(self.engage)
    end
end
...
function Skill:show_tier(index, minimal)
    ...
    if self.engage then
        if self.engage.data.slot ~= self.data.slot then
            self.engage = false
        end
    end 
    ...
    if self.engage then
        if self.engage.stats then
            stats = util.math.add_stats(tier.stats, self.engage.data.stats)
            stats.cooldown = math.max(stats.cooldown, 1)
        end

    end
    
    -- stats
    local text = emoji .. util.table_stats(stats, {key_start = "**", key_end = "**"})
    ...
    if self.engage then
        desc = desc .. "\n" .. heroes_pack:get("feh_ring") .. self.engage.data.description
    end
...
function Skill:get_portrait()
    ...
    elseif self.engage then
        return string.format("database/feh/images/passive/%s.png", self.engage.data.icon)
    end
end
...
function Display:show()
    ...
    local function display(section, artist, resplendent)
        ...
        for i, pair in ipairs(display_order) do
            ...
            local url = self.data.images[pair]

            if resplendent then
                url = self.data.resplendent_images[pair]
    ...
    if self:has_resplendent() then
        ...
        pagebox:button({emoji = "bond", section = 1,
        show = {1}, page = 0})
    end
...
setmetatable(Engage, almanac.Workspace)

Engage.section = almanac.get("database/feh/engage.json")

---------------------------------------------------
-- Keyword --
---------------------------------------------------
Keyword.__index = Keyword
setmetatable(Keyword, almanac.Workspace)

Keyword.section = almanac.get("database/feh/keyword.json")
...
function Keyword:show()
    ...
    --weapons
    if self.data.reference_weapon then
        local weaponbox = Infobox:new({title = self.data.name})
        weaponbox:insert("List of Weapons")
        local TotalNum = 0

        local function weapon_field(name, emoji, emoji2, emoji3, emoji4, data)
            if #data > 0 then
                local text = "" 
                ...
                weaponbox:insert(string.format("%s%s%s%s%s", heroes_pack:get(emoji), heroes_pack:get(emoji2), heroes_pack:get(emoji3), heroes_pack:get(emoji4), name), text, true)
            end
        end
        
        weapon_field("Swords", "Red_Sword", "", "", "", self.data.reference_weapon.Sword)
        weapon_field("Lances", "Blue_Lance", "", "", "", self.data.reference_weapon.Lance)
        weapon_field("Axes", "Green_Axe", "", "", "", self.data.reference_weapon.Axe)
        weapon_field("Staves", "Colorless_Staff", "", "", "", self.data.reference_weapon.Staff)
        weapon_field("Bows", "Red_Bow", "Blue_Bow", "Green_Bow", "Colorless_Bow", self.data.reference_weapon.Bow)
        weapon_field("Daggers", "Red_Dagger", "Blue_Dagger", "Green_Dagger", "Colorless_Dagger", self.data.reference_weapon.Dagger)
        weapon_field("Red Tomes", "Red_Tome", "", "", "", self.data.reference_weapon.Tome.Red_Tome)
        weapon_field("Blue Tomes", "Blue_Tome", "", "", "", self.data.reference_weapon.Tome.Blue_Tome)
        weapon_field("Green Tomes", "Green_Tome", "", "", "", self.data.reference_weapon.Tome.Green_Tome)
        weapon_field("Colorless Tomes", "Colorless_Tome", "", "", "", self.data.reference_weapon.Tome.Colorless_Tome)
        weapon_field("Breaths", "Red_Breath", "Blue_Breath", "Green_Breath", "Colorless_Breath", self.data.reference_weapon.Breath)
        weapon_field("Beasts", "Red_Beast", "Blue_Beast", "Green_Beast", "Colorless_Beast", self.data.reference_weapon.Beast)

        weaponbox:set("footer", string.format("%s Weapons", TotalNum))
        pagebox:page(weaponbox)
        pagebox:button({label = "Weapons", page = pageNum, emoji = heroes_pack:get("feh_weapons")})
        pageNum = pageNum + 1
    end

    --skills
    if self.data.reference_skill then
        local skillsbox = Infobox:new({title = self.data.name})
        skillsbox:insert("List of Passives")
        local TotalNum = 0

        local function skills_field(name, emoji, data)
            if #data > 0 then
                local text = "" 
                ...
                skillsbox:insert(string.format("%s%s", heroes_pack:get("feh_" .. emoji), name), text, true)
            end
        end
        
        skills_field("Assist", "assist", self.data.reference_skill.assist)
        skills_field("Special", "special", self.data.reference_skill.special)
        skills_field("Passive A", "A", self.data.reference_skill.A)
        skills_field("Passive B", "B", self.data.reference_skill.B)
        skills_field("Passive C", "C", self.data.reference_skill.C)
        skills_field("Passive X", "X", self.data.reference_skill.X)
        skills_field("Sacred Seal", "S", self.data.reference_skill.S)
        skills_field("Captain", "Cap", self.data.reference_skill.captain)

        skillsbox:set("footer", string.format("%s Passives", TotalNum))
        pagebox:page(skillsbox)
        pagebox:button({label = "Passives", page = pageNum, emoji = "manual"})
        pageNum = pageNum + 1
    end

    if #self.data.reference_hero >= 1 then
    --duo/harmonic skills
        local herosbox = Infobox:new({title = self.data.name})
        local text3 = ""
        for key, value in pairs(self.data.reference_hero) do
            text3 = text3 .. value .. "\n"

        end
        herosbox:insert("List of Skills", text3)
        herosbox:set("footer", string.format("%s Duo / Harmonic / Emblem Skills", #self.data.reference_hero))
        pagebox:page(herosbox)
        pagebox:button({label = "Skills", page = pageNum, emoji = heroes_pack:get("feh_duo")})
    end
...
function Keyword:get_portrait()
    
    return string.format("database/feh/images/status/%s.png", self.data.icon)
end
